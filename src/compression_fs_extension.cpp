...
inline void CompressionFsOpenSSLVersionScalarFun(DataChunk &args, ExpressionState &state, Vector &result) {
	auto &name_vector = args.data[0];
	UnaryExecutor::Execute<string_t, string_t>(name_vector, result, args.size(), [&](string_t name) {
		return StringVector::AddString(result, "CompressionFs " + name.GetString() + ", my linked OpenSSL version is " +
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	auto compression_fs_openssl_version_scalar_function = ScalarFunction("compression_fs_openssl_version", {LogicalType::VARCHAR},
	                                                             LogicalType::VARCHAR, CompressionFsOpenSSLVersionScalarFun);
